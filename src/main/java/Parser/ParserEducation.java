...
public class ParserEducation {
    private List<Article> articleList;

    public ParserEducation() {
    }
    ...
    private static Document getPage() throws IOException {
        String url = "https://tlgrm.ru/channels/education";
        Document page = Jsoup.parse(new URL(url),15000);
        return page;
    }

    public static void main(String[] args) throws IOException {
        ParserEducation parserEducation = new ParserEducation();
        parserEducation.articleList = new ArrayList<>();//Это главный Лист
        ...
        for (int i = 1; i < nameList.size(); i++) {
            parserEducation.articleList.add(new Article(nameList.get(i),numberList.get(i),linkList.get(i),descriptionList.get(i)));
        }
        parserEducation.articleList.forEach(System.out :: println);
