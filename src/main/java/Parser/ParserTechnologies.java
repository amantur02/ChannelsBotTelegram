...
public class ParserTechnologies {
    private List<Article> articleList;

    public ParserTechnologies() {
    }
    ...
    private static Document getPage() throws IOException {
        String url = "https://tlgrm.ru/channels/tech";
        Document page = Jsoup.parse(new URL(url),15000);
        return page;
    }

    public static void main(String[] args) throws IOException {
        ParserTechnologies parserTechnologies = new ParserTechnologies();

        parserTechnologies.articleList = new ArrayList<>();//Это главный Лист
        ...
        for (int i = 1; i < nameList.size(); i++) {
            parserTechnologies.articleList.add(new Article(nameList.get(i),numberList.get(i),linkList.get(i),descriptionList.get(i)));
        }
        parserTechnologies.articleList.forEach(System.out :: println);
