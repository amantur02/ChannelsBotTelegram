...
public class ParserEconomics {
    private List<Article> articleList;

    public ParserEconomics() {
    }
    ...
    private static Document getPage() throws IOException {
        String url = "https://tlgrm.ru/channels/economics";
        Document page = Jsoup.parse(new URL(url),15000);
        return page;
    }

    public static void main(String[] args) throws IOException {
        ParserEconomics parserEconomics = new ParserEconomics();
        parserEconomics.articleList = new ArrayList<>();//Это главный Лист
        ...
        for (int i = 1; i < nameList.size(); i++) {
            parserEconomics.articleList.add(new Article(nameList.get(i),numberList.get(i),linkList.get(i),descriptionList.get(i)));
        }
        parserEconomics.articleList.forEach(System.out :: println);
