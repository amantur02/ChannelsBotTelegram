...
public class ParserEntertainment {
    private List<Article> articleList;

    public ParserEntertainment() {
    }
    ...
    private static Document getPage() throws IOException {
        String url = "https://tlgrm.ru/channels/entertainment";
        Document page = Jsoup.parse(new URL(url),15000);
        return page;
    }

    public static void main(String[] args) throws IOException {
        ParserEntertainment parserEntertainment = new ParserEntertainment();
        parserEntertainment.articleList = new ArrayList<>();//Это главный Лист
        ...
        for (int i = 1; i < nameList.size(); i++) {
            parserEntertainment.articleList.add(new Article(nameList.get(i),numberList.get(i),linkList.get(i),descriptionList.get(i)));
        }
        parserEntertainment.articleList.forEach(System.out :: println);
