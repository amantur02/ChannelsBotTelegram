...
public class ParserCrypto {
    ...
    private static Document getPage() throws IOException {
        String url = "https://tlgrm.ru/channels/crypto";
        Document page = Jsoup.parse(new URL(url),15000);
        return page;
    }

    public static void main(String[] args) throws IOException {
        ParserCrypto parserCrypto = new ParserCrypto();
        parserCrypto.articleList = new ArrayList<>();//Это главный Лист
        ...
        for (int i = 1; i < nameList.size(); i++) {
            parserCrypto.articleList.add(new Article(nameList.get(i),numberList.get(i),linkList.get(i),descriptionList.get(i)));
        }
        parserCrypto.articleList.forEach(System.out :: println);
